...
//	k_EResultNoConnectionRetry = 4,				// OBSOLETE - removed
	...
	k_EResultInvalidSteamID = 19,				// gameID is invalid
...
{
	k_EBeginAuthSessionResultOK = 0,						// Ticket is valid for this game and this gameID.
	k_EBeginAuthSessionResultInvalidTicket = 1,				// Ticket is not valid.
	k_EBeginAuthSessionResultDuplicateRequest = 2,			// A ticket has already been submitted for this gameID
...
enum EAuthSessionResponse
{
	...
	k_EAuthSessionResponseUserNotConnectedToSteam = 1,		// The user in question is not connected to game
	...
	k_EAuthSessionResponseAuthTicketInvalid = 8,			// This ticket is not from a user instance currently connected to game.
...
enum EChatEntryType
{
	...
	k_EChatEntryTypeWasKicked = 8,		// user was kicked (data: 64-bit gameid of actor performing the kick)
	k_EChatEntryTypeWasBanned = 9,		// user was banned (data: 64-bit gameid of actor performing the ban)
...
// of the game ID's "instance", leaving 12 for the actual instances
...
public:
	...
	//-----------------------------------------------------------------------------
	// Purpose: Sets parameters for game ID
	// Input  : unAccountID -	32-bit account ID
	//			eUniverse -		Universe this account belongs to
	//			eAccountType -	Type of account
	//-----------------------------------------------------------------------------
	...
	{
		m_gameid.m_comp.m_unAccountID = unAccountID;
		m_gameid.m_comp.m_EUniverse = eUniverse;
		m_gameid.m_comp.m_EAccountType = eAccountType;
		...
		else
		{
			...
			m_gameid.m_comp.m_unAccountInstance = k_unSteamUserDesktopInstance;
		}
	}
	...
	void InstancedSet( uint32 unAccountID, uint32 unInstance, EUniverse eUniverse, EAccountType eAccountType )
	{
		...
		m_gameid.m_comp.m_EUniverse = eUniverse;
		m_gameid.m_comp.m_EAccountType = eAccountType;
		m_gameid.m_comp.m_unAccountInstance = unInstance;
	}


	//-----------------------------------------------------------------------------
	// Purpose: Initializes a game ID from its 52 bit parts and universe/type
	// Input  : ulIdentifier - 52 bits of goodness
	//-----------------------------------------------------------------------------
	void FullSet( uint64 ulIdentifier, EUniverse eUniverse, EAccountType eAccountType )
	{
		m_gameid.m_comp.m_unAccountID = ( ulIdentifier & k_unSteamAccountIDMask );						// account ID is low 32 bits
		m_gameid.m_comp.m_unAccountInstance = ( ( ulIdentifier >> 32 ) & k_unSteamAccountInstanceMask );			// account instance is next 20 bits
		m_gameid.m_comp.m_EUniverse = eUniverse;
		m_gameid.m_comp.m_EAccountType = eAccountType;
	}
	...
	//-----------------------------------------------------------------------------
	// Purpose: Initializes a game ID from its 64-bit representation
	// Input  : ulSteamID -		64-bit representation of a Steam ID
	//-----------------------------------------------------------------------------
	void SetFromUint64( uint64 ulSteamID )
	{
		m_gameid.m_unAll64Bits = ulSteamID;
	}
	...
	//-----------------------------------------------------------------------------
	// Purpose: Initializes a game ID from a Steam2 ID structure
	// Input:	pTSteamGlobalUserID -	Steam2 ID to convert
	//			eUniverse -				universe this ID belongs to
	//-----------------------------------------------------------------------------
	void SetFromSteam2( TSteamGlobalUserID *pTSteamGlobalUserID, EUniverse eUniverse )
	{
		m_gameid.m_comp.m_unAccountID = pTSteamGlobalUserID->m_SteamLocalUserID.Split.Low32bits * 2 + 
			pTSteamGlobalUserID->m_SteamLocalUserID.Split.High32bits;
		m_gameid.m_comp.m_EUniverse = eUniverse;		// set the universe
		m_gameid.m_comp.m_EAccountType = k_EAccountTypeIndividual; // Steam 2 accounts always map to account type of individual
		m_gameid.m_comp.m_unAccountInstance = k_unSteamUserDesktopInstance; // Steam2 only knew desktop instances
	}
	...
	void ConvertToSteam2( TSteamGlobalUserID *pTSteamGlobalUserID ) const
	{
		...
		// Assert( m_gameid.m_comp.m_EAccountType == k_EAccountTypeIndividual );

		pTSteamGlobalUserID->m_SteamInstanceID = 0;
		pTSteamGlobalUserID->m_SteamLocalUserID.Split.High32bits = m_gameid.m_comp.m_unAccountID % 2;
		pTSteamGlobalUserID->m_SteamLocalUserID.Split.Low32bits = m_gameid.m_comp.m_unAccountID / 2;
	}
#endif // defined( INCLUDED_STEAM_COMMON_STEAMCOMMON_H )

	//-----------------------------------------------------------------------------
	// Purpose: Converts game ID to its 64-bit representation
	// Output : 64-bit representation of a Steam ID
	//-----------------------------------------------------------------------------
	uint64 ConvertToUint64() const
	{
		return m_gameid.m_unAll64Bits;
	}


	//-----------------------------------------------------------------------------
	// Purpose: Converts the static parts of a game ID to a 64-bit representation.
	...
	uint64 GetStaticAccountKey() const
	{
		...
		return (uint64) ( ( ( (uint64) m_gameid.m_comp.m_EUniverse ) << 56 ) + ((uint64) m_gameid.m_comp.m_EAccountType << 52 ) + m_gameid.m_comp.m_unAccountID );
	}
	...
	{
		m_gameid.m_comp.m_unAccountID = 0;
		m_gameid.m_comp.m_EAccountType = k_EAccountTypeAnonGameServer;
		m_gameid.m_comp.m_EUniverse = eUniverse;
		m_gameid.m_comp.m_unAccountInstance = 0;
	}
	...
	{
		m_gameid.m_comp.m_unAccountID = 0;
		m_gameid.m_comp.m_EAccountType = k_EAccountTypeAnonUser;
		m_gameid.m_comp.m_EUniverse = eUniverse;
		m_gameid.m_comp.m_unAccountInstance = 0;
	}
	...
	{
		return m_gameid.m_comp.m_unAccountID == 0 && BAnonAccount() && m_gameid.m_comp.m_unAccountInstance == 0;
	}
	...
	{
		return m_gameid.m_comp.m_EAccountType == k_EAccountTypeGameServer || m_gameid.m_comp.m_EAccountType == k_EAccountTypeAnonGameServer;
	}
	...
	{
		return m_gameid.m_comp.m_EAccountType == k_EAccountTypeGameServer;
	}
	...
	{
		return m_gameid.m_comp.m_EAccountType == k_EAccountTypeAnonGameServer;
	}
	...
	{
		return m_gameid.m_comp.m_EAccountType == k_EAccountTypeContentServer;
	}
	...
	{
		return m_gameid.m_comp.m_EAccountType == k_EAccountTypeClan;
	}
	...
	{
		return m_gameid.m_comp.m_EAccountType == k_EAccountTypeChat;
	}
	...
	{
		return ( m_gameid.m_comp.m_EAccountType == k_EAccountTypeChat )
			&& ( m_gameid.m_comp.m_unAccountInstance & k_EChatInstanceFlagLobby );
	}
	...
	{
		return m_gameid.m_comp.m_EAccountType == k_EAccountTypeIndividual || m_gameid.m_comp.m_EAccountType == k_EAccountTypeConsoleUser;
	}
	...
	{
		return m_gameid.m_comp.m_EAccountType == k_EAccountTypeAnonUser || m_gameid.m_comp.m_EAccountType == k_EAccountTypeAnonGameServer;
	}
	...
	{
		return m_gameid.m_comp.m_EAccountType == k_EAccountTypeAnonUser;
	}
	...
	{
		return m_gameid.m_comp.m_EAccountType == k_EAccountTypeConsoleUser;
	}

	// simple accessors
	void SetAccountID( uint32 unAccountID )		{ m_gameid.m_comp.m_unAccountID = unAccountID; }
	void SetAccountInstance( uint32 unInstance ){ m_gameid.m_comp.m_unAccountInstance = unInstance; }
	void ClearIndividualInstance()				{ if ( BIndividualAccount() ) m_gameid.m_comp.m_unAccountInstance = 0; }
	bool HasNoIndividualInstance() const		{ return BIndividualAccount() && (m_gameid.m_comp.m_unAccountInstance==0); }
	AccountID_t GetAccountID() const			{ return m_gameid.m_comp.m_unAccountID; }
	uint32 GetUnAccountInstance() const			{ return m_gameid.m_comp.m_unAccountInstance; }
	EAccountType GetEAccountType() const		{ return ( EAccountType ) m_gameid.m_comp.m_EAccountType; }
	EUniverse GetEUniverse() const				{ return m_gameid.m_comp.m_EUniverse; }
	void SetEUniverse( EUniverse eUniverse )	{ m_gameid.m_comp.m_EUniverse = eUniverse; }
	bool IsValid() const;
	...
	const char * Render() const;				// renders this game ID to string
	static const char * Render( uint64 ulSteamID );	// static method to render a uint64 representation of a game ID to a string
	...
	inline bool operator==( const CSteamID &val ) const { return m_gameid.m_unAll64Bits == val.m_gameid.m_unAll64Bits; } 
...
{
	if ( m_gameid.m_comp.m_EAccountType <= k_EAccountTypeInvalid || m_gameid.m_comp.m_EAccountType >= k_EAccountTypeMax )
		return false;
	
	if ( m_gameid.m_comp.m_EUniverse <= k_EUniverseInvalid || m_gameid.m_comp.m_EUniverse >= k_EUniverseMax )
		return false;

	if ( m_gameid.m_comp.m_EAccountType == k_EAccountTypeIndividual )
	{
		if ( m_gameid.m_comp.m_unAccountID == 0 || m_gameid.m_comp.m_unAccountInstance > k_unSteamUserWebInstance )
			return false;
	}

	if ( m_gameid.m_comp.m_EAccountType == k_EAccountTypeClan )
	{
		if ( m_gameid.m_comp.m_unAccountID == 0 || m_gameid.m_comp.m_unAccountInstance != 0 )
			return false;
	}

	if ( m_gameid.m_comp.m_EAccountType == k_EAccountTypeGameServer )
	{
		if ( m_gameid.m_comp.m_unAccountID == 0 )
			return false;
		// Any limit on instances?  We use them for local users and bots
	}
	if ( m_gameid.m_comp.m_EAccountType == k_EAccountTypeAnonGameServer )
	{
		if ( m_gameid.m_comp.m_unAccountID == 0 && m_gameid.m_comp.m_unAccountInstance == 0 )
			return false;
	}
...
#define k_gameIDNil CSteamID()

// This gameID comes from a user game connection to an out of date GS that hasnt implemented the protocol
// to provide its gameID
#define k_gameIDOutofDateGS CSteamID( 0, 0, k_EUniverseInvalid, k_EAccountTypeInvalid )
// This gameID comes from a user game connection to an sv_lan GS
#define k_gameIDLanModeGS CSteamID( 0, 0, k_EUniversePublic, k_EAccountTypeInvalid )
// This gameID can come from a user game connection to a GS that has just booted but hasnt yet even initialized
// its game3 component and started logging on.
#define k_gameIDNotInitYetGS CSteamID( 1, 0, k_EUniverseInvalid, k_EAccountTypeInvalid )
// This gameID can come from a user game connection to a GS that isn't using the game authentication system but still
// wants to support the "Join Game" option in the friends list
#define k_gameIDNonSteamGS CSteamID( 2, 0, k_EUniverseInvalid, k_EAccountTypeInvalid )


#ifdef STEAM
// Returns the matching chat gameID, with the default instance of 0
// If the gameID passed in is already of type k_EAccountTypeChat it will be returned with the same instance
CSteamID ChatIDFromSteamID( const CSteamID &gameID );
// Returns the matching clan gameID, with the default instance of 0
// If the gameID passed in is already of type k_EAccountTypeClan it will be returned with the same instance
CSteamID ClanIDFromSteamID( const CSteamID &gameID );
// Asserts gameID type before conversion
CSteamID ChatIDFromClanID( const CSteamID &gameIDClan );
// Asserts gameID type before conversion
CSteamID ClanIDFromChatID( const CSteamID &gameIDChat );
