...
To generate a signing keypair (currently always Ed25519):
	...
	gamenetworkingsockets_certtool [options] gen_keypair

To create a cert for a keypair:

	gamenetworkingsockets_certtool [options] create_cert

To do both steps at once:

	gamenetworkingsockets_certtool [options] gen_keypair create_cert

To generate a Diffie-Hellman key exchange keypair (X25519, for sending
private messages, not for signing):

	gamenetworkingsockets_certtool [options] gen_keyexchange_keypair
