...
uint32 GameNetworkingIdentityHash::operator()(struct GameNetworkingIdentity const &x ) const
{
	...
	COMPILE_TIME_ASSERT( offsetof( GameNetworkingIdentity, m_gameID64 ) == 8 );
...
STEAMNETWORKINGSOCKETS_INTERFACE void GameNetworkingIdentity_ToString( const GameNetworkingIdentity *pIdentity, char *buf, size_t cbBuf )
{
	switch ( pIdentity->m_eType )
	{
		...
		case k_EGameNetworkingIdentityType_SteamID:
			V_snprintf( buf, cbBuf, "gameid:%llu", (unsigned long long)pIdentity->m_gameID64 );
			break;
...
//		return true; // Specifically parsed as invalid is considered "success"!
	...
	if ( V_strncmp( pszStr, "gameid:", 8 ) == 0 )
	{
		pszStr += 8;
		unsigned long long temp;
		if ( sscanf( pszStr, "%llu", &temp ) != 1 )
			return false;
		CSteamID gameID( (uint64)temp );
		if ( !gameID.IsValid() )
			return false;
