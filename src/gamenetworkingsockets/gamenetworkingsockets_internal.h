...
#endif
#include <gns/gamenetworkingtypes.h>
...
#include <gamenetworkingsockets_messages_certs.pb.h>
#include <gns/igamenetworkingutils.h> // for the rendering helpers
...
inline bool IsValidSteamIDForIdentity( CSteamID gameID )
{
	return gameID.GetAccountID() != 0 && ( gameID.BIndividualAccount() || gameID.BGameServerAccount() );
}

inline bool IsValidSteamIDForIdentity( uint64 gameid64 ) { return IsValidSteamIDForIdentity( CSteamID( gameid64 ) ); }
...
#define BGameNetworkingIdentityToProtobuf( identity, msg, field_identity_string, field_identity_legacy_binary, field_legacy_game_id, errMsg ) ( \
		( (identity).GetSteamID64() ? (void)(msg).set_ ## field_legacy_game_id( (identity).GetSteamID64() ) : (void)0 ), \
...
#define GameNetworkingIdentityToProtobuf( identity, msg, field_identity_string, field_identity_legacy_binary, field_legacy_game_id ) \
	{ SteamDatagramErrMsg identityToProtobufErrMsg; \
		if ( !BGameNetworkingIdentityToProtobuf( identity, msg, field_identity_string, field_identity_legacy_binary, field_legacy_game_id, identityToProtobufErrMsg ) ) { \
...
extern bool BGameNetworkingIdentityFromLegacySteamID( GameNetworkingIdentity &identity, uint64 legacy_game_id, SteamDatagramErrMsg &errMsg );
...
// 0  No data
// >0 OK
#define GameNetworkingIdentityFromProtobuf( identity, msg, field_identity_string, field_identity_legacy_binary, field_legacy_game_id, errMsg ) \
	( \
		...
		: (msg).has_ ##field_legacy_game_id() ? ( BGameNetworkingIdentityFromLegacySteamID( identity, (msg).field_legacy_game_id(), errMsg ) ? +1 : -1 ) \
...
{
	return GameNetworkingIdentityFromProtobuf( result, msgCert, identity_string, legacy_identity_binary, legacy_game_id, errMsg );
}
