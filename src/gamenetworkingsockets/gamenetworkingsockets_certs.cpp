...
{
	if ( msgIdentity.has_game_id() )
	{
		if ( !IsValidSteamIDForIdentity( msgIdentity.game_id() ) )
		{
			V_sprintf_safe( errMsg, "Invalid SteamID %llu", (unsigned long long)msgIdentity.game_id() );
			return -1;
		}
		identity.SetSteamID64( msgIdentity.game_id() );
		return 1;
	}
...
bool BGameNetworkingIdentityFromLegacySteamID( GameNetworkingIdentity &identity, uint64 legacy_game_id, SteamDatagramErrMsg &errMsg )
{
	if ( !IsValidSteamIDForIdentity( legacy_game_id ) )
	{
		V_sprintf_safe( errMsg, "Invalid SteamID %llu (in legacy field)", legacy_game_id );
		return false;
	}
	identity.SetSteamID64( legacy_game_id );
	return true;
}
...
bool BGameNetworkingIdentityToProtobufInternal( const GameNetworkingIdentity &identity, std::string *strIdentity, CMsgGameNetworkingIdentityLegacyBinary *msgIdentityLegacyBinary, SteamDatagramErrMsg &errMsg )
{
	switch ( identity.m_eType )
	{
		...
		case k_EGameNetworkingIdentityType_SteamID:
			Assert( identity.m_cbSize == sizeof(identity.m_gameID64) );
			if ( !IsValidSteamIDForIdentity( identity.m_gameID64 ) )
			{
				V_sprintf_safe( errMsg, "Invalid SteamID %llu", identity.m_gameID64 );
				return false;
			}
			msgIdentityLegacyBinary->set_game_id( identity.m_gameID64 );
			break;
