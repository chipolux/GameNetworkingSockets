...
//-----------------------------------------------------------------------------
// Purpose: Initializes a game ID from a string
// Input  : pchSteamID -		text representation of a Steam ID
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Purpose: Initializes a game ID from a Steam2 ID string
...
bool CSteamID::SetFromSteam2String( const char *pchSteam2ID, EUniverse eUniverse )
{
	...
	TSteamGlobalUserID game2ID;
	game2ID.m_SteamInstanceID = 0;
	game2ID.m_SteamLocalUserID.Split.High32bits = 0;
	game2ID.m_SteamLocalUserID.Split.Low32bits	= 0;

	const char *pchTSteam2ID = pchSteam2ID;

	// Customer support is fond of entering game IDs in the following form:  STEAM_n:x:y
	...
	int cFieldConverted = sscanf( pchTSteam2ID, "%hu:%u:%u%c", &game2ID.m_SteamInstanceID, 
		&game2ID.m_SteamLocalUserID.Split.High32bits, &game2ID.m_SteamLocalUserID.Split.Low32bits, &cExtraCharCheck );

	// Validate the conversion ... a special case is game2 instance ID 1 which is reserved for special DoD handling
	if ( cExtraCharCheck != 0 || cFieldConverted == EOF || cFieldConverted < 2 || ( cFieldConverted < 3 && game2ID.m_SteamInstanceID != 1 ) )
		return false;

	// Now convert to game ID from the Steam2 ID structure
	SetFromSteam2( &game2ID, eUniverse );
	return true;
}
#endif

//-----------------------------------------------------------------------------
// Purpose: Renders the game ID to a buffer.  NOTE: for convenience of calling
...
const char * CSteamID::Render() const
{
	...
	if ( k_EAccountTypeAnonGameServer == m_gameid.m_comp.m_EAccountType )
	{
		V_snprintf( pchBuf, k_cBufLen, "[A:%u:%u:%u]", m_gameid.m_comp.m_EUniverse, m_gameid.m_comp.m_unAccountID, m_gameid.m_comp.m_unAccountInstance );
	}
	else if ( k_EAccountTypeGameServer == m_gameid.m_comp.m_EAccountType )
	{
		V_snprintf( pchBuf, k_cBufLen, "[G:%u:%u]", m_gameid.m_comp.m_EUniverse, m_gameid.m_comp.m_unAccountID );
	}
	else if ( k_EAccountTypeMultiseat == m_gameid.m_comp.m_EAccountType )
	{
		V_snprintf( pchBuf, k_cBufLen, "[M:%u:%u:%u]", m_gameid.m_comp.m_EUniverse, m_gameid.m_comp.m_unAccountID, m_gameid.m_comp.m_unAccountInstance );
	} 
	else if ( k_EAccountTypePending == m_gameid.m_comp.m_EAccountType )
	{
		V_snprintf( pchBuf, k_cBufLen, "[P:%u:%u]", m_gameid.m_comp.m_EUniverse, m_gameid.m_comp.m_unAccountID );
	} 
	else if ( k_EAccountTypeContentServer == m_gameid.m_comp.m_EAccountType )
	{
		V_snprintf( pchBuf, k_cBufLen, "[C:%u:%u]", m_gameid.m_comp.m_EUniverse, m_gameid.m_comp.m_unAccountID );
	}
	else if ( k_EAccountTypeClan == m_gameid.m_comp.m_EAccountType )
	{
		// 'g' for "group"
		V_snprintf( pchBuf, k_cBufLen, "[g:%u:%u]", m_gameid.m_comp.m_EUniverse, m_gameid.m_comp.m_unAccountID );
	}
	else if ( k_EAccountTypeChat == m_gameid.m_comp.m_EAccountType )
	{
		if ( m_gameid.m_comp.m_unAccountInstance & k_EChatInstanceFlagClan )
		{
			V_snprintf( pchBuf, k_cBufLen, "[c:%u:%u]", m_gameid.m_comp.m_EUniverse, m_gameid.m_comp.m_unAccountID );
		}
		else if ( m_gameid.m_comp.m_unAccountInstance & k_EChatInstanceFlagLobby )
		{
			V_snprintf( pchBuf, k_cBufLen, "[L:%u:%u]", m_gameid.m_comp.m_EUniverse, m_gameid.m_comp.m_unAccountID );
		}
		else // Anon chat
		{
			V_snprintf( pchBuf, k_cBufLen, "[T:%u:%u]", m_gameid.m_comp.m_EUniverse, m_gameid.m_comp.m_unAccountID );
		}
	}
	else if ( k_EAccountTypeInvalid == m_gameid.m_comp.m_EAccountType )
	{
		V_snprintf( pchBuf, k_cBufLen, "[I:%u:%u]", m_gameid.m_comp.m_EUniverse, m_gameid.m_comp.m_unAccountID );
	}
	else if ( k_EAccountTypeIndividual == m_gameid.m_comp.m_EAccountType )
	{
		if ( m_gameid.m_comp.m_unAccountInstance != k_unSteamUserDesktopInstance )
			V_snprintf( pchBuf, k_cBufLen, "[U:%u:%u:%u]", m_gameid.m_comp.m_EUniverse, m_gameid.m_comp.m_unAccountID, m_gameid.m_comp.m_unAccountInstance );
		else
			V_snprintf( pchBuf, k_cBufLen, "[U:%u:%u]", m_gameid.m_comp.m_EUniverse, m_gameid.m_comp.m_unAccountID );
	}
	else if ( k_EAccountTypeAnonUser == m_gameid.m_comp.m_EAccountType )
	{
		V_snprintf( pchBuf, k_cBufLen, "[a:%u:%u]", m_gameid.m_comp.m_EUniverse, m_gameid.m_comp.m_unAccountID );
	}
	else
	{
		V_snprintf( pchBuf, k_cBufLen, "[i:%u:%u]", m_gameid.m_comp.m_EUniverse, m_gameid.m_comp.m_unAccountID );
...
//-----------------------------------------------------------------------------
// Purpose: Renders the passed-in game ID to a buffer.  NOTE: for convenience of calling
//			code, this code returns a pointer to a static buffer and is NOT thread-safe.
...
{
	CSteamID gameID( ulSteamID );
	return gameID.Render();
}


//-----------------------------------------------------------------------------
// Purpose: some gameIDs are for internal use only
// This is really debug code, but we run with asserts on in retail, so ...
//-----------------------------------------------------------------------------
bool CSteamID::BValidExternalSteamID() const
{
	if ( m_gameid.m_comp.m_EAccountType == k_EAccountTypePending )
		return false;
	if ( m_gameid.m_comp.m_EAccountType != k_EAccountTypeAnonGameServer && m_gameid.m_comp.m_EAccountType != k_EAccountTypeContentServer && m_gameid.m_comp.m_EAccountType != k_EAccountTypeAnonUser )
	{
...
#ifdef STEAM
//-----------------------------------------------------------------------------
// Purpose:	Returns the matching chat gameID, with the default instance of 0
...
{
	if ( gameID.GetEAccountType() == k_EAccountTypeChat )
		return gameID;

	return ChatIDFromClanID( gameID );
}
...
{
	if ( gameID.GetEAccountType() == k_EAccountTypeClan )
		return gameID;

	return ClanIDFromChatID( gameID );
}
...
{
	Assert( gameIDClan.GetEAccountType() == k_EAccountTypeClan );

	return CSteamID( gameIDClan.GetAccountID(), k_EChatInstanceFlagClan, gameIDClan.GetEUniverse(), k_EAccountTypeChat );
}
...
{
	Assert( gameIDChat.GetEAccountType() == k_EAccountTypeChat );
	Assert( k_EChatInstanceFlagClan & gameIDChat.GetUnAccountInstance() );

	return CSteamID( gameIDChat.GetAccountID(), 0, gameIDChat.GetEUniverse(), k_EAccountTypeClan );
}
